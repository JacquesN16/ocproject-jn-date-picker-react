...
const getDaysForCalendar = (date = new Date(), calendarSize = 42) => {
  ...
  const calendarStartDate = dateFns.subDays(monthStartDate, numOfDaysToAddBefore);
  const calendarEndDate = dateFns.addDays(monthEndDate, calendarSize - daysInMonth - numOfDaysToAddBefore);
  const interval = {
    start: calendarStartDate,
    end: calendarEndDate
  };
  dateFns.eachDayOfInterval(interval).forEach(date => visibleCalendarDays.push(date));
